...
namespace SearchАrraySkripnik
{
    ...
    {   
        ...
        {
            listBox1.Items.Clear();
            /*На уроці валеології ліцеїсти 9 класу задали провести спостереження за температурою повітря за останні 2 тижні. Учень вимірював температуру вдень (о 14:00) і вранці (о 6:00). Вам необхідно написати методи, що дозволяють
             * сформувати 2 масиву спостережень, заповнивши дані за допомогою датчика випадкових в інтервалі від -5 до 10 градусів (використовувати вже створений);
             * масиви спостережень вивести на форму за допомогою елементів управління ListBox(використовувати вже створений);
             * визначити, в які дні була мінімальна денна температура і вивести номери цих днів на форму за допомогою елемента управління ListBox;(два методи один визначає мінімальне значення, а інший друкує знайдені значення)
             * визначити, в який день ранкова температура вперше опустилася нижче 0;
             * визначити день, ранкова температура в який була найбільш близькою до середнього значення ранкової температури за весь період спостережень;(два методи – середня температура і найближче значення)
             * сформувати третій масив, який буде містити різниця між денною і ранкової температурами і вивести отриманий масив на форму за допомогою елемента управління ListBox;
            ...
            double lowest = 15;
            double min2 = 0;
            listBox1.Items.Add("ARR1(14): ");
            displayArray(ref arr1);
            listBox1.Items.Add("ARR2(6): ");
            displayArray(ref arr2);
            dif(n,arr2,arr1);
            minT(n, ref min1, arr1);
            LowerZero(n, ref zero2);
            avT(n, ref av, arr2);
            closeToAvT(n, ref min2, arr2, ref lowest, av);
            tGrewUp(n,arr1,ref count_increase1);
            biggerT(n,t,ref count2);
        }
    }
}
